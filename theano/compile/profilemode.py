...
class ProfileMode(Mode):
    ...
    def __get_local_time(self):
        rval = 0
        for ps in list(self.profile_stats.values()):
    ...
    def print_summary(self, **kwargs):
        ...
        fct_call = dict([(fn, ps.fct_callcount)
            for (fn, ps) in list(self.profile_stats.items())])
        ...
        for (i, n), t in list(apply_time.items()):
            if t == 0:
                print(i, n)
    ...
    def print_summary_(fct_name, compile_time, fct_call_time, fct_call,
        ...
        op_cimpl = {}
        sop_apply = {}
        for (i, a), t in list(apply_time.items()):
            op = a.op
            ...
            op_time[op] += t
            nb_call = [v for k, v in list(fct_call.items()) if k.maker.fgraph is a.fgraph][0]
        ...
        sotimes = [(t*100/local_time, t, a, sop_cimpl[a], sop_call[a], sop_op[a], sop_apply[a]) for a, t in list(sop_time.items())]
        sotimes.sort()
        sotimes.reverse()
        tot = 0
        ...
        if print_apply:
            ...
            atimes = [(t*100/local_time, t, a, [v for k, v in list(fct_call.items()) if k.maker.fgraph is a[1].fgraph][0]) for a, t in list(apply_time.items())]
            atimes.sort()
            atimes.reverse()
            tot = 0
