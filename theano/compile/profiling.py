...
class ProfileStats(object):
    ...
    def class_time(self):
        ...
        rval = {}
        for node, t in list(self.apply_time.items()):
            typ = type(node.op)
    ...
    def class_callcount(self):
        ...
        # timing is stored by node, we compute timing by class on demand
        rval = {}
        for node, count in list(self.apply_callcount.items()):
            typ = type(node.op)
            rval.setdefault(typ, 0)
            rval[typ] += count
        return rval
    ...
    def fill_node_total_time(self, node, total_times):
        ...
        for parent in node.get_parents():
            if parent.owner in list(self.apply_time.keys()):
                if parent.owner not in list(total_times.keys()):
    ...
    def compute_total_times(self):
        ...
        rval = {}
        for node in list(self.apply_time.keys()):
            if node not in rval:
    ...
    def summary_class(self, file=sys.stderr, N=None):
        ...
        otimes = [(t * 100 / local_time,
                  ...
                  for clas, t in list(class_time.items())]
    ...
    def summary_ops(self, file=sys.stderr, N=None):
        ...
        otimes = [(t * 100 / local_time,
                   ...
                   op_call.get(op, 0),
                   op_apply.get(op, 0))
                  for op, t in list(op_time.items())]
        otimes.sort(key=lambda t: (t[1], t[4], t[5]), reverse=True)
        tot = 0
        print('Ops', file=file)
        print('---', file=file)
    ...
    def summary_nodes(self, file=sys.stderr, N=None):
        ...
        atimes = []
        for a, t in list(self.apply_time.items()):
    ...
    def summary_memory(self, file, N=None):
        ...
        for node in list(self.apply_callcount.keys()):
            ...
            for out in node.outputs:
                if out in list(self.variable_shape.keys()):
...
if 0:  # old code still to be ported from ProfileMode
    def long_print(self, file=sys.stderr, fct_name=None, message=None,
        ...
        sotimes = [(t * 100 / local_time, t, a, sop_c[a],
                    sop_call[a], sop_op[a]) for a, t in list(sop_time.items())]
    ...
    def print_diff_summary(self, other, n_apply_to_print=15,
        ...
        def diff_dict(a_time, b_time_):
            ...
            for a, ta in list(a_time.items()):
                r.setdefault(a, 0)
                tb = b_time.pop(a, 0)
                r[a] += ta - tb

            # they are missing in a
            for a, t in list(b_time.items()):
                r.setdefault(a, 0)
                r[a] += t
            return r
